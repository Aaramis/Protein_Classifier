...
def test_build_labels(fake_data):
    _, targets = fake_data
...
def test_get_amino_acid_frequencies(fake_data):
...
def test_build_vocab(fake_data):
    sequence, _ = fake_data
...
def test_sequence_dataset(fake_data):
    sequence, targets = fake_data
    word2id = build_vocab(sequence)
    fam2label = build_labels(targets)
